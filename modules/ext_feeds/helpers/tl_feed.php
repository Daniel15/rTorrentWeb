...
/**
 * Helper for the torrent leech feed
 *
 * The TL feeds are evil :
 *  - Have no GUID so we use the link property
 *  - They do not store torrents as enclosures, but in the link property
 */
class tl_feed
{
	public static function get_torrents($url)
	{
		...
		foreach ($raw_feed_items as $raw_feed_item)
		{
			$formatted_feed[] = array(
				...
				'guid' => $raw_feed_item['link'],
				'torrent_url' => $raw_feed_item['link'],
			);
		}
